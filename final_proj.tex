...
The university aims to have $\xi$ number of upperclassmen living on campus. Thus, the incentive, or $K$, that the university provides must have only one nash equilbria, such that $\xi$ number of upperclassmen choose to live on campus. Thus, we must derive some relationship between $\Lambda, \phi, K$ to enforce this notion. To simplify notation, let $x$ be the number of upperclassmen who choose the live on campus. 
\subsubsection*{title}
