...
\end{subprob}
\begin{subsol}
We can consider 3 cases: $|a| < 1, |a| = 1, |a| > 1$. Note that if $|a| = 1$, we have the linear difference equation $x_{t_1} = x_t + b$, which implies that there is not stable solution, and thus approachs infinity. If $|a| < 1$ or $|a| > 1$, this means that the exponetial term grows slower and faster than the linear term respectively. However, as noted in \textbf{1}, we can see that if the slope of graph $|ax_t^{a-1}| < 1$, we will asymptocally approach a solution. Else, we may diverge and not approach a steady state solution. \todo{Suppose conditions under local stability, what conditions connverge above or below . show 100}
\todo{Do the graphically}
\end{subsol}
\begin{subprob}
\end{subprob}
\begin{subsol}
...
\begin{subsol}
    ...
    \end{figure}
    The steady state changes to approximately 2.45. Both trajectories seem to either steeply rise and fall or fall and rise for intial values of 0.1 and 10 resepctively. This is constrast to the other set of parameters as in the original case, there are no kinks in the graph. \todo{Check, different trajectories. Monotone convergence, osciallation}
\end{subsol}
\begin{subprob}
...
\begin{subsol}
    ...
    Geometric mean of passches index and \todo{Same basket}. Compare geometric with arithmetic mean, compare AM - GM inqualitu Hold basket fixed, not taking into account sub effect
\end{subsol}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
