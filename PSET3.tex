...
\]
Note that with $\alpha >1$, we can see that the above quantity is always negative, which implies that average cost is strictly less than that of marginal cost, which means the firm would want to produce infinite amounts. If $\alpha < 1$, we know that 
