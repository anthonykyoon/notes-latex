...
\end{subprob}
\begin{subsol}
    We can note that consumption does indeed jump right away so that the individual can reach the steady state path, as the change in $G$ is sudden and thus, the change in consumption must change immediately. We can see this at $t = 0$, the consumption will drop to $A$ and then go to $B$ then to $C$ to encourage the economy to be on the steady state path. 
    \begin{figure}[H]
        \centering
