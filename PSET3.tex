...
\end{subsol}
\begin{subprob}
    Recall this equation, we can see that:
    \[
        \frac{c_{t+1}}{c_t} = \alpha k^{\alpha - 1}_{t+1} + 1 - \delta - \gamma - \rho
    \]
    This implies that if $k$ decreases, than $c$ must increases, hence promoting the north west direction. This cannot happen in equilbrium as $k_t \to 0$ and consumption $c_t \to \infty$, as noted from above, which violates the resource constraints. 
\end{subprob}
